...
def get_config():
    ...
    openai_base_url = os.environ.get('OPENAI_BASE_URL', 'https://fanyi.963312.xyz')
    return jsonify({
        'default_api': 'aliyun',
        'aliyun': {
...
def translate_text():
    ...
    try:
        ...
        if api_type == 'deeplx':
            ...
            return translate_with_openai(text, source_lang, target_lang, data)
        else:
...
def translate_long_text_openai(text, source_lang, target_lang, api_key, model, base_url, chunk_size):
    """分块翻译长文本（OpenAI兼容API）"""
    try:
        # 按段落分割文本
        paragraphs = text.split('\n\n')
        ...
        print(f"[OpenAI翻译] 分成 {len(chunks)} 块进行翻译")
        ...
        for i, chunk in enumerate(chunks):
            print(f"[OpenAI翻译] 翻译第 {i+1}/{len(chunks)} 块...")
        ...
        # 合并所有翻译结果
        final_translation = '\n\n'.join(translated_chunks)
        print(f"[OpenAI翻译] 所有块翻译完成，总长度: {len(final_translation)} 字符")
...
def translate_with_openai(text, source_lang, target_lang, data):
    """使用OpenAI兼容API翻译"""
    try:
        # 客户端可传入api_key、model和base_url；若未提供，则使用服务端环境变量
        api_key = data.get('api_key') or os.environ.get('OPENAI_API_KEY', '')
        model = data.get('model') or os.environ.get('OPENAI_DEFAULT_MODEL', 'gpt-3.5-turbo')
        base_url = data.get('base_url') or os.environ.get('OPENAI_BASE_URL', 'https://fanyi.963312.xyz')

        if not api_key:
            return jsonify({'error': '请先配置OpenAI API Key'}), 400

        print(f"[OpenAI翻译] 使用模型: {model}, API Key: {api_key[:10]}..., Base URL: {base_url}")

        # 构建翻译提示词
        source_lang_name = LANGUAGE_NAMES.get(source_lang, source_lang)
        target_lang_name = LANGUAGE_NAMES.get(target_lang, target_lang)

        print(f"[OpenAI翻译] 翻译方向: {source_lang_name} -> {target_lang_name}")
        print(f"[OpenAI翻译] 文本长度: {len(text)} 字符")

        # 如果文本太长，分块翻译
        max_chunk_size = 2000  # 每块最大字符数
        if len(text) > max_chunk_size:
            print(f"[OpenAI翻译] 文本过长，分块翻译...")
            return translate_long_text_openai(text, source_lang, target_lang, api_key, model, base_url, max_chunk_size)
        ...
        for attempt in range(max_retries):
            try:
                print(f"[OpenAI翻译] 尝试 {attempt + 1}/{max_retries}...")
                response = requests.post(url, headers=headers, json=payload, timeout=120)

                if response.status_code == 200:
                    result = response.json()
                    translated_text = result['choices'][0]['message']['content'].strip()
                    print(f"[OpenAI翻译] 翻译成功，结果长度: {len(translated_text)} 字符")
                    ...
                    error_msg = f"OpenAI API返回错误: {response.status_code} - {response.text}"
                    print(f"[错误] {error_msg}")
                    if attempt == max_retries - 1:
                        return jsonify({'error': error_msg}), 500
            except requests.exceptions.Timeout:
                print(f"[警告] 第 {attempt + 1} 次尝试超时")
                if attempt == max_retries - 1:
                    return jsonify({'error': '翻译请求超时，请尝试使用更快的模型或减少PDF页数'}), 500
            except Exception as e:
                print(f"[错误] 第 {attempt + 1} 次尝试失败: {str(e)}")
                if attempt == max_retries - 1:
                    raise
        ...
        error_str = str(e)
        print(f"[错误] OpenAI翻译出错: {error_str}")
        import traceback
        traceback.print_exc()

        # 提供更友好的错误提示
        if 'Invalid API key' in error_str or 'Incorrect API key' in error_str:
            error_msg = 'OpenAI API Key无效，请检查是否正确复制'
        elif 'Model not found' in error_str:
            error_msg = f'模型 {model} 不存在，请检查模型名称'
        elif 'Insufficient balance' in error_str or 'balance' in error_str.lower():
            error_msg = '账户余额不足，请前往控制台充值'
        elif 'Rate limit' in error_str:
            error_msg = '请求频率过高，请稍后再试'
        elif 'timeout' in error_str.lower():
            error_msg = '请求超时，请检查网络连接'
        else:
            error_msg = f'OpenAI翻译出错: {error_str}'
